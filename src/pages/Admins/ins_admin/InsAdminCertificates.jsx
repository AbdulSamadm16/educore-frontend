...
export default function InsAdminCertificates() {
  ...
  const fetchTemplates = async () => {
    try {
      setLoading(true);
      const res = await apiClient.get('/institution-admin/certificate-templates');
  ...
  const toggleActiveStatus = async (template) => {
    try {
      ...
      await apiClient.patch(`/institution-admin/certificate-templates/${templateId}`, { isActive: newStatus });
      toast.success(`Template ${newStatus ? 'activated' : 'deactivated'} successfully`);
      fetchTemplates();
    } catch (err) {
      if (err.response?.data?.code === 'ACTIVE_LIMIT_EXCEEDED') {
        toast.error('Maximum active institution certificate templates limit of 4 has been reached. Deactivate another template first.', { duration: 6000 });
      } else {
    ...
    <div className="animate-in fade-in duration-500 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/60 text-sm mt-1">Manage and design institution-wide certificates.</p>
        </div>
        <button
          onClick={() => openModal()}
          ...
          {templates.map(template => (
            <div key={template.id || template._id} className="glass-panel border border-white/5 rounded-[24px] overflow-hidden group">
              ...
              <div className="p-5">
                ...
                <p className="text-xs text-white/50 mb-4 font-mono">v{template.version || 1} • {template.scope || 'Institution'}</p>
